  ...
  try {
    ...
    el.innerHTML = `
      <div class="fcard"><h2 style="margin:0">Your invite link</h2>${link}
        <div class="lbstats four"><div><span class="note">Invited</span><b>${j.referred}</b></div><div><span class="note">Your share</span><b>${pct(j.tierBps)}</b></div><div><span class="note">Earned</span><b>${usd(j.earnedUsd)}</b></div><div><span class="note">Your points</span><b>${Math.round(j.points).toLocaleString("en-US")}</b></div></div>
      ...
      <div class="fcard"><h3 style="margin:0">Earnings</h3>${table}${payouts}</div>
