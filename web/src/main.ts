...
import { STOCK_NAMES, bucketLabel, bucketName, fmtMove, fmtUsd, issuerOf, loadPrices, loadStocks, question, tokenSymbol } from "./stocks";
import { bucketColor, esc, fmtTs, mountTopbar, renderCatnav, tickerBadge, timeLeft, trackStocks } from "./ui";

const qs = new URLSearchParams(location.search);
const state = { stock: qs.get("stock") ?? "all", issuer: qs.get("issuer") ?? "all", status: qs.get("status") ?? "open", sort: qs.get("sort") ?? "closing", q: qs.get("q") ?? "" };
const DEFAULTS: Record<string, string> = { stock: "all", issuer: "all", status: "open", sort: "closing", q: "" };
...
mountTopbar({ active: state.stock, q: state.q, onSearch: (q) => { state.q = q; update(); }, onStock: (s) => { state.stock = s; update(); } });
