...
async function render(owner: PublicKey | null) {
  ...
  try {
    ...
    settledEl.innerHTML = `<div class="scroll"><table class="tbl"><thead><tr><th>Market</th><th>Your stakes</th><th>Outcome</th><th class="r">Paid</th><th>Tx</th></tr></thead><tbody>${j.settled.map((s: any) => {
      const m = byKey.get(s.market); const tok = m ? esc(tokenSymbol(m)) : "";
      const amt = (x: number) => (m ? fmtAmt(m, x) : String(x));
      const bets = s.amounts.map((a: string, i: number) => (Number(a) ? `${m ? esc(label(m, i)) : "range " + i}: <span class="mono">${amt(Number(a))}</span>` : "")).filter(Boolean).join("<br>");
      const outcome = s.status === 3 ? "Voided" : m ? `${esc(label(m, Number(s.outcome)))} (${fmtMove(Number(s.observed))})` : `range ${esc(s.outcome)}`;
