...
import { STOCK_ORDER, loadPrices, loadStocks, tokensOf } from "./stocks";
...
async function init() {
  ...
  trackStocks(ms);
  list.innerHTML = STOCK_ORDER.flatMap((s) => tokensOf(s).map((t) => `<span class="ftok">${tickerBadge(s, false, true)}<b>2 ${esc(t.token)}</b><span class="note">${esc(t.issuer)}</span></span>`)).join("") + `<span class="ftok"><b>+ 0.01 SOL</b><span class="note">for network fees</span></span>`;
  render();
}
function render() {
  ...
  const s = getSession();
  box.innerHTML = s
    ? `<div class="thead"><b>Get test stocks</b><span class="note mono">${short(s.publicKey)}</span></div>
       <p class="note" style="margin:0">Sends the tokens on the left to this wallet. One claim per wallet per day.</p>
       <button class="primary big" id="fbtn"${busy ? " disabled" : ""}>${busy ? "Sending…" : "Get test stocks"}</button><div id="fmsg">${result}</div>`
    : `<div class="thead"><b>Get test stocks</b></div>
